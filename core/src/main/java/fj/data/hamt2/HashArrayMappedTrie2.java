...
import static fj.data.hamt2.BitSet2.longBitSet;
 ...
 */
public final class HashArrayMappedTrie2<K, V> {

    private final Seq<Node2<K, V>> seq;
    private final BitSet2 bitSet;
     ...
     */
    private HashArrayMappedTrie2(final BitSet2 bs, final Seq<Node2<K, V>> s, final Equal<K> e, final Hash<K> h) {
        bitSet = bs;
        seq = s;
        hash = h;
        equal = e;
     ...
     */
    public static <K, V> HashArrayMappedTrie2<K, V> empty(final Equal<K> e, final Hash<K> h) {
        return new HashArrayMappedTrie2<>(BitSet2.empty(), Seq.empty(), e, h);
    }

    /**
     * Create and empty trie keyed by integer.
     */
    public static <V> HashArrayMappedTrie2<Integer, V> emptyKeyInteger() {
     ...
     */
    private static <K, V> HashArrayMappedTrie2<K, V> hamt(final BitSet2 bs, final Seq<Node2<K, V>> s, final Equal<K> e, final Hash<K> h) {
        return new HashArrayMappedTrie2<>(bs, s, e, h);
    }
    ...
    public Option<V> find(final K k, final int lowIndex, final int highIndex) {
        BitSet2 bs1 = longBitSet(hash.hash(k)).range(lowIndex, highIndex);
        ...
        if (!b) {
            ...
            final Node2<K, V> oldNode = seq.index(index);
     ...
     */
    public HashArrayMappedTrie2<K, V> set(final K k, final V v) {
     ...
     */
    public HashArrayMappedTrie2<K, V> set(final List<P2<K, V>> list) {
     ...
     */
    private HashArrayMappedTrie2<K, V> set(final K k, final V v, final int lowIndex, final int highIndex) {
        final BitSet2 bs1 = longBitSet(hash.hash(k)).range(lowIndex, highIndex);
        ...
        if (!b) {
            // append new node
            final Node2<K, V> sn1 = Node2.p2Node(p(k, v));
            return HashArrayMappedTrie2.hamt(bitSet.set(i), seq.insert(index, sn1), equal, hash);
        } else {
            final Node2<K, V> oldNode = seq.index(index);
            final Node2<K, V> newNode = oldNode.match(n -> {
                if (equal.eq(n._1(), k)) {
                    return Node2.p2Node(p(k, v));
                } else {
                    final HashArrayMappedTrie2<K, V> e = HashArrayMappedTrie2.<K, V>empty(equal, hash);
                    final HashArrayMappedTrie2<K, V> h1 =  e.set(n._1(), n._2(), lowIndex + BITS_IN_INDEX, highIndex + BITS_IN_INDEX);
                    final HashArrayMappedTrie2<K, V> h2 = h1.set(k, v, lowIndex + BITS_IN_INDEX, highIndex + BITS_IN_INDEX);
                    return Node2.hamtNode(h2);
                }
            }, hamt -> Node2.hamtNode(hamt.set(k, v, lowIndex + BITS_IN_INDEX, highIndex + BITS_IN_INDEX))
            );
    ...
    public String toString() {
        return Show.hamtShow2(Show.<K>anyShow(), Show.<V>anyShow()).showS(this);
    }

    /**
     * Performs a left-fold reduction across this trie.
     */
    public <B> B foldLeftOnNode(F2<B, Node2<K, V>, B> f, B b) {
     ...
     * Performs a left-fold reduction across this trie.
     */
    public <B> B foldLeft(F2<B, P2<K, V>, B> f, F2<B, HashArrayMappedTrie2<K, V>, B> g, B b) {
